...
int add_iptables_rules()
{
    ...
    if (opt_protect_http_protocol) {
        ...
        sprintf(cmd, "iptables -A INPUT -p tcp --sport 6666 --tcp-flags RST RST -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing SYN
        sprintf(cmd, "iptables -A OUTPUT -p tcp --dport 6666 --tcp-flags SYN,ACK SYN -m mark ! --mark %d -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming SYN-ACK
        sprintf(cmd, "iptables -A INPUT -p tcp --sport 6666 --tcp-flags SYN,ACK SYN,ACK -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing ACK
        sprintf(cmd, "iptables -A OUTPUT -p tcp --dport 6666 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m length --length 0:80 -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing HTTP 'GET ' request
        sprintf(cmd, "iptables -A OUTPUT -p tcp --dport 6666 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0=0x47455420' -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing HTTP 'POST' request
        sprintf(cmd, "iptables -A OUTPUT -p tcp --dport 6666 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0=0x504F5354' -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept incoming HTTP 'HTTP' response
        sprintf(cmd, "iptables -A INPUT -p tcp --sport 6666 --tcp-flags SYN,ACK,RST ACK -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0=0x48545450' -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
    }
...
int remove_iptables_rules()
{
    ...
    if (opt_protect_http_protocol) {
        // monitor incoming RST
        sprintf(cmd, "iptables -D INPUT -p tcp --sport 6666 --tcp-flags RST RST -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing SYN
        sprintf(cmd, "iptables -D OUTPUT -p tcp --dport 6666 --tcp-flags SYN,ACK SYN -m mark ! --mark %d -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming SYN-ACK
        sprintf(cmd, "iptables -D INPUT -p tcp --sport 6666 --tcp-flags SYN,ACK SYN,ACK -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing ACK
        sprintf(cmd, "iptables -D OUTPUT -p tcp --dport 6666 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m length --length 0:80 -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing HTTP 'GET ' request
        sprintf(cmd, "iptables -D OUTPUT -p tcp --dport 6666 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0=0x47455420' -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing HTTP 'POST' request
        sprintf(cmd, "iptables -D OUTPUT -p tcp --dport 6666 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0=0x504F5354' -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept incoming HTTP 'HTTP' response
        sprintf(cmd, "iptables -D INPUT -p tcp --sport 6666 --tcp-flags SYN,ACK,RST ACK -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0=0x48545450' -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
    }
    if (opt_protect_https_protocol) {
        // monitor incoming RST
...
int process_tcp_packet(struct mypacket *packet, char inout)
{
    ...
    //if (dport == 6666 && is_blocked_ip(dip)) {
